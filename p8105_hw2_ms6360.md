...
For Mr. Trash Wheel dataset, there are 16 columns (variables) and 454
rows (observations). As for Professor Trash Wheel, there are 15 columns
(variables) and 71 rows (observations). Once combined, there are 16
columns (variables) and 525 rows (observations). Variables in this
dataset include dumpster, month, year, date, weight_tons,
volume_cubic_yards, plastic_bottles, polystyrene, cigarette_butts,
glass_bottles, grocery_bags, chip_bags, sports_balls, homes_powered,
x15, name. Within these variables, the weight of trash collected in
tons, number of trash collected (plastic bottles, polystyrene, cigarette
...
democratic presidents, governors, senators, and representatives.
...
Finally for the `unemployment` dataset, there are 816 rows
(observations) and 3 columns (variables).The range of years is (1948,
2015). Variables in this dataset (year, month, percent_unemployment)
include represents the percentage of unemployment associated with the
month.
